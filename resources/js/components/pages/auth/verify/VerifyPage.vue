<template>
  <div class="flex flex-col items-center justify-center min-h-screen px-4">
    ...
    <div class="text-center mb-8 mt-8">
      <h1 class="text-white text-2xl mb-2">Xác minh</h1>
    ...
    <div class="bg-white rounded-xl shadow-lg p-8 max-w-md w-full text-center space-y-6">
      ...
      <p class="text-gray-600">
        ...
        <span class="font-medium text-blue-600">{{ login }}</span>
      </p>
...
const resendOtp = async () => {
  loading.value = true
  try {
    await userStore.resendOtp({ login: login.value })
...
const submitOtp = async () => {
  ...
  try {
    ...
    const res = await userStore.verifyOtp({ login: login.value, otp })
    toast.success('Xác minh thành công')
    setTimeout(() => {
      router.push({ path: '/complete-registration', query: { login: login.value } })
    }, 1500)
  } catch (error) {
